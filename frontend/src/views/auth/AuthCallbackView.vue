...
<template>
  <div
    class="auth-callback-page min-h-screen flex items-center justify-center p-4"
  >
    <Card class="w-full max-w-md">
      <CardHeader class="text-center">
        <CardTitle class="flex items-center justify-center gap-2">
          <img
            src="/src/assets/logo/Hydroline_Logo_Short.svg"
            alt="Logo"
            class="w-8 h-8"
          />
          登录处理中
        </CardTitle>
        <CardDescription> 正在处理第三方登录信息... </CardDescription>
      </CardHeader>
      ...
      <CardContent class="text-center space-y-6">
        ...
        <div v-if="isLoading" class="space-y-4">
          <div class="flex justify-center">
            <span class="material-icons animate-spin text-4xl text-blue-600"
              >refresh</span
            >
          </div>
        ...
        <div v-else-if="success" class="space-y-4">
          <div class="flex justify-center">
            <span class="material-icons text-4xl text-green-600"
              >check_circle</span
            >
          </div>
          <div>
        ...
        <div v-else-if="error" class="space-y-4">
          ...
          <Button @click="goHome" class="w-full"> 返回首页 </Button>
        </div>
      </CardContent>
    </Card>
  </div>
